...
export async function getAllAccount(req: Request, res: Response, next: NextFunction) { //get value through query params
    const path = req.path;
    try {
        ...
        if (time_start) {
            ...
            time_start = dateToTimestamp(time_start as string);
        }
        if (time_end) {
            if (!timeRegex.test(time_end as string)) return res.json(returnError('400', 'date invalid', {}, path));
            time_end = dateToTimestamp(time_end as string);
        }
